...
def find_available_slots(
    ...
    room_type = None if room_type in (None, "", "Any") else str(room_type)
    ...
    day_options_dyn, time_slots_dyn = _get_schedule_dimensions(schedule_df)
    day_order = {d: i for i, d in enumerate(day_options_dyn)}
    ...
    for _, room in rooms.iterrows():
        ...
        for day in day_options_dyn:
            for time in time_slots_dyn:
...
def get_building_overview_kpis(rooms_df: pd.DataFrame, schedule_df: pd.DataFrame) -> dict:
    ...
    lecture_halls = int((rooms_df["Type"].str.contains("Lecture", case=False)).sum() if not rooms_df.empty else 0)
    computer_labs = int((rooms_df["Type"].str.contains("Lab", case=False)).sum() if not rooms_df.empty else 0)
...
def update_schedule_entry(crn: str, new_day: str, new_time: str, new_room: str) -> bool:
    """Updates a schedule entry in the CSV file for drag-and-drop operations."""
    try:
        schedule_df = pd.read_csv(SCHEDULE_CSV)
        schedule_df["CRN"] = schedule_df["CRN"].astype(str)
        # Ensure Room_ID is a string column so we can assign string values without dtype errors
        schedule_df["Room_ID"] = schedule_df["Room_ID"].astype(str)
        
        idx = schedule_df[schedule_df["CRN"] == crn].index
        if len(idx) == 0:
            return False
            
        schedule_df.loc[idx, "Day"] = new_day
        schedule_df.loc[idx, "Time"] = new_time
        schedule_df.loc[idx, "Room_ID"] = new_room
